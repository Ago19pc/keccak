...
void KeccakF1600_StatePermute(keccak_state_t * state){
    ...
    for (uint64_t i = 0; i < 24; i++) {
	    ...
	    s3 = vsetq_lane_u64(temp2 << 2 | temp2 >> 62, s3, 0);
	    vsetq_lane_u64(0, s3, 1);

        temp2 = ((uint64_t*)(&s0))[1]; // 12
        ((uint64_t*)(&s0))[1] = temp1 << 14 | temp1 >> 50;
        temp1 = ((uint64_t*)(&s1))[7]; // 13
        ((uint64_t*)(&s1))[7] = temp2 << 27 | temp2 >> 37;
        temp2 = ((uint64_t*)(&s2))[7]; // 14
        ((uint64_t*)(&s2))[7] = temp1 << 41 | temp1 >> 23;
        temp1 = ((uint64_t*)(&s2))[6]; // 15
        ((uint64_t*)(&s2))[6] = temp2 << 56 | temp2 >> 8;
        temp2 = ((uint64_t*)(&s1))[3]; // 16
        ((uint64_t*)(&s1))[3] = temp1 << 8 | temp1 >> 56;
        temp1 = ((uint64_t*)(&s1))[1]; // 17
        ((uint64_t*)(&s1))[1] = temp2 << 25 | temp2 >> 39;
        temp2 = ((uint64_t*)(&s0))[4]; // 18
        ((uint64_t*)(&s0))[4] = temp1 << 43 | temp1 >> 21;
        temp1 = ((uint64_t*)(&s2))[1]; // 19
        ((uint64_t*)(&s2))[1] = temp2 << 62 | temp2 >> 2;
        temp2 = ((uint64_t*)(&s1))[5]; // 20
        ((uint64_t*)(&s1))[5] = temp1 << 18 | temp1 >> 46;
        temp1 = ((uint64_t*)(&s2))[5]; // 21
        ((uint64_t*)(&s2))[5] = temp2 << 39 | temp2 >> 25;
        temp2 = ((uint64_t*)(&s1))[2]; // 22
        ((uint64_t*)(&s1))[2] = temp1 << 61 | temp1 >> 3;
