...
void keccakF1600_StatePermute(keccak_state_t * state){
    ...
    for (uint64_t i = 0; i < 24; i++) {
        ...
        s3 = veorq_u64(s3, v7);
        //ro pi
        uint64x2_t temp1 = s1[2][0];
        s1[2][0] = s0[1][0] << 1 | s0[1][0] >> 63;
        uint64x2_t temp2 = s0[3][1];
        s0[3][1] = temp1 << 3 | temp1 >> 61;
        temp1 = s1[3][0];
        s1[3][0] = temp2 << 6 | temp2 >> 58;
        temp2 = s2[1][0];
        s2[1][0] = temp1 << 10 | temp1 >> 54;
        temp1 = s2[2][0];
        s2[2][0] = temp2 << 15 | temp2 >> 49;
        temp2 = s0[3][0];
        s0[3][0] = temp1 << 21 | temp1 >> 43;
        temp1 = s0[1][1];
        s0[1][1] = temp2 << 28 | temp2 >> 36;
        temp2 = s2[0][0];
        s2[0][0] = temp1 << 36 | temp1 >> 28;
        temp1 = s2[1][1];
        s2[1][1] = temp2 << 45 | temp2 >> 19;
        temp2 = s3[0];
        s3[0] = temp1 << 55 | temp1 >> 9;
        temp1 = s0[0][1];
        s0[0][1] = temp2 << 2 | temp2 >> 62;
        temp2 = s1[3][1];
        s1[3][1] = temp1 << 14 | temp1 >> 50;
        temp1 = s2[3][1];
        s2[3][1] = temp2 << 27 | temp2 >> 37;
        temp2 = s2[3][0];
        s2[3][0] = temp1 << 41 | temp1 >> 23;
        temp1 = s1[1][1];
        s1[1][1] = temp2 << 56 | temp2 >> 8;
        temp2 = s1[0][1];
        s1[0][1] = temp1 << 8 | temp1 >> 56;
        temp1 = s0[2][0];
        s0[2][0] = temp2 << 25 | temp2 >> 39;
        temp2 = s2[0][1];
        s2[0][1] = temp1 << 43 | temp1 >> 21;
        temp1 = s1[2][1];
        s1[2][1] = temp2 << 62 | temp2 >> 2;
        temp2 = s2[2][1];
        s2[2][1] = temp1 << 18 | temp1 >> 46;
        temp1 = s1[1][0];
        s1[1][0] = temp2 << 39 | temp2 >> 25;
        temp2 = s0[2][1];
        s0[2][1] = temp1 << 61 | temp1 >> 3;
        s0[1][0] = temp2 << 44 | temp2 >> 20;
        //chi
        v18 = vextq_u64(s1[3], s3, 8);
        v6 = vbicq_u64(s2[0], v18);
        uint64x2_t v17 = vextq_u64(s1[3], s3, 8);
        v5 = vbicq_u64(v17, s2[3]);
        v3 = vbicq_u64(s2[3], s2[2]);
        s2[3] = veorq_u64(s2[3], v6);
        v18 = vtrn1q_u64(s1[3], s1[2]);
        v17 = vextq_u64(s1[0], s1[3], 8);
        v7 = vbicq_u64(v17, v18);
        v18 = vtrn2q_u64(s0[1], s1[2]);
        v17 = vextq_u64(s0[2], s1[2], 8);
        v6 = vbicq_u64(v17, v18);
        v18 = vtrn1q_u64(s0[1], s0[0]);
        v17 = vtrn1q_u64(s0[2], s0[1]);
        v1 = vbicq_u64(v17, v18);
        v18 = vextq_u64(s0[0], s1[0], 8);
        v17 = vtrn1q_u64(s0[0], s1[1]);
        v0 = vbicq_u64(v17, v18);
        v18 = vextq_u64(s1[0], s2[0], 8);
        v17 = vextq_u64(s1[1], s2[1]);
        v4 = vbicq_u64(v17, v18);
        v18[0] = s2[0][1];
        v17[0] = s2[1][1];
        v2 = vbicq_u64(v17, v18);
        s3 = veorq_u64(s3, v2);
        v2 = vbicq_u64(s2[2], s2[1]);
        s2[0] = veorq_u64(s2[0], v2);
        v2 = vbicq_u64(s0[3], s0[2]);
        s2[1] = veorq_u64(s2[1], v3);
        s2[2] = veorq_u64(s2[2], v5);
        v17 = vextq_u64(s0[0], s1[0], 8);
        v3 = vbicq_u64(v17, s0[3]);
        v17 = vtrn2q_u64(s0[1], s1[2]);
        v5 = vbicq_u64(v17, s1[1]);
        s0[0] = veorq_u64(s0[0], v1);
        s0[1] = veorq_u64(s0[1], v2);
        s0[2] = veorq_u64(s0[2], v3);
        s0[3] = veorq_u64(s0[3], v0);
        s1[0] = veorq_u64(s1[0], v5);
        s1[1] = veorq_u64(s1[1], v6);
        s1[2] = veorq_u64(s1[2], v7);
        s1[3] = veorq_u64(s1[3], v4);
        //iota
        uint64x2_t roundConstant;
        roundConstant[0] = round_constants[i]; //forse segfault perché roundConstant va inizializzato prima?
        s0[0] = veorq_u64(s0[0], roundConstant);
    }
